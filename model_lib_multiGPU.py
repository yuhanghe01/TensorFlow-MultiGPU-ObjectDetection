...
Note: This file is implemented for multiple GPU training
"""
...
def split_features_and_labels( features, labels, GPU_num ):
    """split features and labels dict w.r.t GPU_num
    
    :param features: features dict
    :param labels: labels dict
    :param GPU_num: GPU num to train the model
    :return: splitted features and labels list
    """
    features_list = list()
    labels_list = list()

    for key_tmp, val_tmp in features.items():
        features[ key_tmp ] = tf.split( value = val_tmp, num_or_size_splits = GPU_num, axis = 0 )

    features_key_list = [ key_tmp for key_tmp in features.keys() ]

    for key_tmp, val_tmp in labels.items():
        labels[ key_tmp ] = tf.split( value = val_tmp, num_or_size_splits = GPU_num, axis = 0 )

    labels_key_list = [ key_tmp for key_tmp in labels.keys() ]

    for i in xrange( GPU_num ):
        feature_dict_tmp = dict()
        for key_tmp in features_key_list:
            feature_dict_tmp[ key_tmp ] = features[ key_tmp ][i]
        features_list.append( feature_dict_tmp )
        label_dict_tmp = dict()
        for key_tmp in labels_key_list:
            label_dict_tmp[ key_tmp ] = labels[ key_tmp ][i]
        labels_list.append( label_dict_tmp )
...
def tower_loss( scope, features, labels, detection_model, train_config ):
    ...
    def model_fn(features, labels, mode, params=None):
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            with tf.variable_scope( tf.get_variable_scope() ):
                feature_list, label_list = split_features_and_labels( features, labels, train_config.GPU_num )
                for i in xrange( train_config.GPU_num ):
                    with tf.device( '/gpu:%d'%i ):
                        with tf.name_scope('%s_%d'%('tower', i)) as scope:
                            loss = tower_loss( scope = scope, features = feature_list[i], labels = label_list[i], detection_model = detection_model, train_config = train_config )
                            tf.get_variable_scope().reuse_variables()
                            grads = training_optimizer.compute_gradients(loss=loss )
                            if isinstance( clip_gradients_value, float ):
                                grads = clip_gradients_by_norm( grads, clip_gradients_value )
            ...
            with tf.control_dependencies( tf.get_collection( tf.GraphKeys.UPDATE_OPS ) ):
                apply_gradient_op = training_optimizer.apply_gradients( grads_and_vars = grad_avg, global_step = global_step )
