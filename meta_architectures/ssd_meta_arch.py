...
class SSDMetaArch(model.DetectionModel):
  ...
  def predict(self, preprocessed_inputs, true_image_shapes):
    ...
    if self._feature_extractor.is_keras_model:
      ...
      with slim.arg_scope([slim.batch_norm],
        ...
        #                       [preprocessed_inputs]):
        with tf.variable_scope( name_or_scope = self._extract_features_scope,
    ...
    if self._box_predictor.is_keras_model:
      ...
      with slim.arg_scope([slim.batch_norm],
        ...
        predictor_results_dict = self._box_predictor.predict(
            feature_maps, self._anchor_generator.num_anchors_per_location(), scope = 'BoxPredictor')
    predictions_dict = {
  ...
  def restore_map(self,
    ...
    if fine_tune_checkpoint_type == 'detection':
      ...
      tf.logging.info('begin to log out global vars ...' )
      for variable in tf.global_variables():
        var_name = variable.op.name
        tf.logging.info( 'global var_name = {}'.format( var_name ) )
        #print( var_name )
